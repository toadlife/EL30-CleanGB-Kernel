...
# Setup su binary
if busybox [ ! -f /system/bin/su ]; then
...
#make room
if busybox [ ! -f "/system/app/Superuser.apk" ] && busybox [ ! -f "/data/app/Superuser.apk" ] && busybox [ ! -f "/data/app/com.noshufou.android.su" ]; then
	if busybox [ -f "/system/app/Asphalt5_DEMO_SAMSUNG_D700_Sprint_ML_330.apk" ]; then
		busybox rm /system/app/Asphalt5_DEMO_SAMSUNG_D700_Sprint_ML_330.apk
	fi
	if busybox [ -f "/system/app/FreeHDGameDemos.apk" ]; then
...
if busybox [ -d /system/etc/init.d ]
then
